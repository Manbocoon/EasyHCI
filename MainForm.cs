...
namespace EasyHCI
{
    public partial class MainForm : MaterialSkin.Controls.MaterialForm
    {
        ...
        StringBuilder NewText1 = new StringBuilder(null, 40), NewText2 = new StringBuilder(null, 40), ErrMsg = new StringBuilder(null, 250), finalRecord = new StringBuilder(null, 800), tSetting = new StringBuilder(null, 800);
        ...
        { 
            ...
            if (notAuto.Checked == false)
            {
                ...
                // 실행 정보 계산 시작 //
                //                     //
                hcimem = (ushort)((freeMemMB - 280) / CPUThreads);
                hcinum = CPUThreads;
                while (hcimem > tmem)
                {
                    hcinum++;
                    hcimem = (ushort)((freeMemMB - 280) / hcinum);
                }
            }
            ...
            { //멤테 개수만큼 반복
                ...
                // 창 숨기기 //
                if (mtHide.Checked == true) { ShowWindow(hci[i1], 0); }
            ...
            while (true)
            {
                ...
                if (minValue > Convert.ToUInt16(HCIMax.Text))
                {
                    ...
                    // 끝날 때 스크린샷
                    if (capAfter.Checked == true)
                    { Screenshot(); }

                    
                    // 만약 설정이 멤테 종료라면 //
                    if (closeMem.Checked == true)
                    {
                        KillMemtest();
        ...
        {// 메모리 사용량 표시
            while (true)
            {
                ...
                cls = null; instances = null;
                Gc1++;
                if (Gc1 >= 150) { Gc1 = 0; GC.Collect(); } //3분마다 가비지컬렉터 실행
        ...
        { //테스트 시작버튼
            ...
            if (File.Exists(Application.StartupPath + "\\Resources\\memtest.exe") == false)
        ...
        { //수동설정 라디오버튼
                HCIMemory.Enabled = true; HCINText.Enabled = true; dLB1.Enabled = true; dLB2.Enabled = true;
        }
        ...
        { //자동설정 라디오버튼
            HCIMemory.Enabled = false; HCINText.Enabled = false; dLB1.Enabled = false; dLB2.Enabled = false;
        }
        ...
        {//프로그램 종료신호 보내기 (기록 작성)
            ...
            if (StartTestThread.IsAlive == true)
        ...
        { //프로그램 시작 시 모든 작업 완료 전까지 잠시 숨김
            ...
            // 프로그램 이름 EasyHCI에서 바꾸면 종료 //
            if (Application.ExecutablePath.Remove(0, Application.StartupPath.Length + 1).ToLower() != "easyhci.exe") { 
                MessageBox.Show("프로그램 이름을 EasyHCI로 바꿔주세요.", "알림", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Environment.Exit(0);
            }
            ...
            tmemText.Location = new Point(1000, 300);
           ...
           tempN = 0; hciMem = 0;
            ...
            if (File.Exists(Application.StartupPath + "\\Resources\\Settings.txt"))
            {
                ...
                if (tfile1[2].Remove(0, tfile1[2].IndexOf(":") + 2) == "Manual") { notAuto.Checked = true; HCIMemory.Enabled = true; HCINText.Enabled = true; dLB1.Enabled = true; dLB2.Enabled = true; }
        ...
        { //폼 아래로 당기면 일부 항목 같이 커지기
            LView1.Size =  new Size(313, this.Size.Height - 213);
            passRate.Location = new Point(4, this.Size.Height - 115);
            memGap.Location = new Point(95, this.Size.Height - 115);
            elTime.Location = new Point(185, this.Size.Height - 115);
